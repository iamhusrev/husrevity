...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-dvh overflow-hidden xl:flex bg-husrev-cream dark:bg-husrev-ink">
      ...
      >
        <AppHeader />
        <main className="relative grain min-h-0 flex-1 overflow-y-auto p-4 md:p-6">
          {/*
            min-h-full + flex flex-col is the key to making both layout
            modes coexist:
              - "fill-viewport" pages (Calendar / Gmail / AI / Vault /
                Reminders) use `flex-1 min-h-0` on their root and fill the
                remaining space after the prompt (when shown).
