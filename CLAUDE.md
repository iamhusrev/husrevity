...
- Multi-tenancy: every domain query filters by `ownerId` (current user). Service methods take `ownerId` as the first param. **Exception: `project` and `task`** — projects support multi-user collaboration (`project_member` rows with role `OWNER`/`EDITOR`/`VIEWER`), so those two modules gate access via `ProjectAccessService.requireAccess(userId, projectId, minRole)` instead of a raw `ownerId` filter. A personal task (`task.projectId === null`) still stays strictly owner-scoped — only project-attached tasks are shared. Project identity is id-based (`/api/projects/:projectId`, no more `:code` routes) precisely because `code` is only unique per-owner and collides once a project is shared.
